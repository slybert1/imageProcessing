...
p, q = ndimage.measurements.center_of_mass(smallArray)

#g = px.argmax()
#h = py.argmax()

#must smooth the gaussian fit in order to find the correct maximum:

#function to interpolate the data:
f = interpolate.interp1d(xAxis, px)

#resample with more samples:
xx = np.linspace(-75, 75, 10000)
...
print(g)
#window = scipy.signal.gaussian(200, 150)
